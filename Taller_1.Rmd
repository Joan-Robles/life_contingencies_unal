...
---
# Productos
Antes de construir la función correspondiente a cada producto, 
La función general es `get_premium_insurance()`, sus parámetros como `price_case_11()`.
## Punto 1
...
Se construye una gráfica para comparar el crecimiento del valor asegurado de los productos de los puntos 2 y 5 para la persona asegurada base.
...
# Formato largo
df_long <- rbind(
  data.frame(time = df$time, value = df$p2, type = "Punto 2: aritmético intra-anual"),
  data.frame(time = df$time, value = df$p5, type = "Punto 5: geométrico anual")
)
...
Además, se compara el valor de la prima única de los productos de los puntos 2 y 5 para una misma edad de emisión y distintos plazos de aseguramiento. Para ello se utiliza la función general `get_premium_insurance()`, manteniendo en cada caso la estructura propia del producto. 
